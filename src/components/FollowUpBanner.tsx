...
export default function FollowUpBanner() {
  ...
  return (
    <div className="bg-amber-950/25 border border-amber-800/40 rounded-2xl p-4 space-y-2.5">
      ...
      <div className="space-y-2">
        {items.map(item => (
          <div key={item.id}
            className="bg-amber-950/20 border border-amber-800/30 rounded-xl px-3.5 py-2.5">
            <div className="flex items-center justify-between gap-2">
              <div className="min-w-0">
                <p className="text-xs font-semibold text-amber-200">
                  {FOCUS_AREA_LABELS[item.focusArea]}
                  {item.urgency === 'overdue' && (
                    <span className="ml-2 text-red-400 font-normal">overdue</span>
                  )}
                </p>
                <p className="text-[11px] text-amber-600 mt-0.5 line-clamp-1">{item.agreedActions}</p>
              </div>
      ...
      <p className="text-[11px] text-amber-700">Your manager will check in on these during your next coaching session.</p>
    </div>
  )
}
